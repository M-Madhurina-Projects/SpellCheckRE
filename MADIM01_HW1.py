##NOTE - functions are called in the end. 
import re
...
def checker(input_string):
     ...
     no_digit_input = re.sub(r'\d+','',input_lower)
     no_punctuation_input = re.sub("[^A-Za-z\s'-]+","",no_digit_input)
     
     processed_input = no_punctuation_input.replace("-", " ") 
     ...
     if(len(misspelt)==0):
          ...
          for misspelt_word in misspelt:
               ...
               #using for loop to go through each of the wors in the dictionary
               for dict_word in dictionary_words:
                    ...
                    #suggestions is a dictionary. we are adding key-value pairs to it as we calculate levenshtein distance
                    #calculated distance is key, and the word will be the value 
               
               minimum_key=min(sorted(suggestions))
               #sorted sorts the dictionary by key. minimum_key gives the lowest key, that is, lowest levenshtein distance
               
               suggested_words.extend(suggestions[minimum_key])
               #using extend to add all the words with lowest distance to the list
               #this helps with suggesting the words with the lowest distance 
               
               suggested_words.extend(suggestions[minimum_key+1])
               #adding the words with next lowest distance as well 

               #removing a suggested word from the list if its length is not same as the length of the missplet word
...
#add_suggestions functions takes the calculated dist(distance between the misspelt word and each of the words from the dictionary) 
#and saves it into an empty dictionary
def add_suggestions(dictionary,key,add_values):
     if key not in dictionary:
        #creating a list for the value
        dictionary[key] = list()
     dictionary[key].append(add_values)
     return dictionary  


#remove # below and call the functions
